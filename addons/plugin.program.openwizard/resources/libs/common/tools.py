...
@contextmanager
def busy_dialog():
    # Trong Kodi 21, busydialognocancel đã được thay thế bằng busydialog
    # Sử dụng try-except để hỗ trợ cả phiên bản cũ và mới
    try:
...
def _check_url(url, cred):
    ...
    if _is_url(url):
        try:
            # Sử dụng GET thay vì HEAD vì một số máy chủ có thể không hỗ trợ HEAD
            # Đặt timeout cao hơn để tránh lỗi timeout
            response = requests.head(url, headers={'user-agent': CONFIG.USER_AGENT},
                                    allow_redirects=True, auth=cred, timeout=30.0)
            ...
            if response.status_code < 300:
                ...
                return _check_url(response.headers['Location'], cred)
            elif response.status_code == 401:
                logging.log("URL requires authentication for {0}: Status code [{1}]".format(url, response.status_code), level=xbmc.LOGDEBUG)
                return 'auth'
            else:
                # Thử phương thức GET nếu HEAD không thành công
                try:
                    get_response = requests.get(url, headers={'user-agent': CONFIG.USER_AGENT},
                                            allow_redirects=True, auth=cred, timeout=30.0, stream=True)
                    # Đóng kết nối ngay lập tức để tránh tải toàn bộ nội dung
                    get_response.close()

                    if get_response.status_code < 400:
                        logging.log("URL check with GET passed for {0}: Status code [{1}]".format(url, get_response.status_code), level=xbmc.LOGDEBUG)
                        return True
                    else:
                        logging.log("URL check with GET failed for {0}: Status code [{1}]".format(url, get_response.status_code), level=xbmc.LOGDEBUG)
                        return False
                except Exception as e:
                    logging.log("URL GET check error for {0}: [{1}]".format(url, e), level=xbmc.LOGDEBUG)
                    return False
        except requests.exceptions.ConnectTimeout:
            logging.log("URL check connection timeout for {0}".format(url), level=xbmc.LOGDEBUG)
            return False
        except requests.exceptions.ReadTimeout:
            logging.log("URL check read timeout for {0}".format(url), level=xbmc.LOGDEBUG)
            return False
        except requests.exceptions.ConnectionError:
            logging.log("URL check connection error for {0}".format(url), level=xbmc.LOGDEBUG)
            return False
        except Exception as e:
...
def open_url(url, stream=False, check=False, cred=None, count=0, max_retries=3, cache=True):
    import requests
    import time
    import os
    from resources.libs.common import logging
    ...
    # Kiểm tra cache nếu được yêu cầu và không phải là stream
    cache_file = None
    if cache and not stream and not check and url == CONFIG.BUILDFILE:
        cache_dir = os.path.join(CONFIG.PLUGIN_DATA, 'cache')
        if not os.path.exists(cache_dir):
            try:
                os.makedirs(cache_dir)
            except Exception as e:
                logging.log(f"Không thể tạo thư mục cache: {e}", level=xbmc.LOGERROR)
                cache = False

        if cache:
            cache_file = os.path.join(cache_dir, 'buildfile_cache.txt')
            cache_time_file = os.path.join(cache_dir, 'buildfile_cache_time.txt')

            # Kiểm tra xem cache có tồn tại và còn mới không (dưới 30 phút)
            if os.path.exists(cache_file) and os.path.exists(cache_time_file):
                try:
                    ...
                    # Nếu cache còn mới (dưới 30 phút)
                    if time.time() - cache_time < 1800:  # 30 phút = 1800 giây
                        logging.log(f"Sử dụng cache cho {url}", level=xbmc.LOGINFO)
                    ...
                    logging.log(f"Lỗi khi đọc cache: {e}", level=xbmc.LOGERROR)

    # Nếu không có cache hoặc cache không hợp lệ, tiếp tục tải từ URL
    retry_count = 0
        ...
        try:
            ...
            if not valid:
                if retry_count < max_retries:
                    retry_count += 1
                    xbmc.sleep(1000)  # Đợi 1 giây trước khi thử lại
                    continue
                else:
                    return False
            else:
                ...
                response = requests.get(url, headers=user_agent, timeout=timeout, stream=stream, auth=cred)

                if response.status_code == 401:
                    retry = dialog.yesno(CONFIG.ADDONTITLE, 'Tên người dùng hoặc mật khẩu không hợp lệ. Bạn có muốn thử lại không?', yeslabel='Thử Lại', nolabel='Hủy')
                    ...
                    if retry and count < 3:
                        ...
                        dialog.ok(CONFIG.ADDONTITLE, 'Xác thực thất bại.')
                        return False

                # Lưu vào cache nếu thành công và là BUILDFILE
                if cache_file and response.status_code == 200:
                    try:
                        with open(cache_file, 'w', encoding='utf-8') as f:
                            f.write(response.text)
                        with open(cache_time_file, 'w') as f:
                            f.write(str(time.time()))
                        logging.log(f"Đã lưu {url} vào cache", level=xbmc.LOGINFO)
                    except Exception as e:
                        logging.log(f"Không thể lưu cache: {e}", level=xbmc.LOGERROR)

                return response

        except requests.exceptions.ConnectTimeout:
            logging.log(f"Kết nối đến {url} bị timeout (lần {retry_count+1}/{max_retries+1})", level=xbmc.LOGWARNING)
        except requests.exceptions.ReadTimeout:
            logging.log(f"Đọc từ {url} bị timeout (lần {retry_count+1}/{max_retries+1})", level=xbmc.LOGWARNING)
        except requests.exceptions.ConnectionError:
            logging.log(f"Lỗi kết nối đến {url} (lần {retry_count+1}/{max_retries+1})", level=xbmc.LOGWARNING)
        except Exception as e:
            logging.log(f"Lỗi khi mở {url}: {e} (lần {retry_count+1}/{max_retries+1})", level=xbmc.LOGWARNING)

        retry_count += 1
        if retry_count <= max_retries:
            xbmc.sleep(2000 * retry_count)  # Tăng thời gian chờ giữa các lần thử

    # Nếu tất cả các lần thử đều thất bại nhưng có cache cũ, sử dụng cache cũ
    if cache_file and os.path.exists(cache_file):
        try:
            logging.log(f"Sử dụng cache cũ cho {url} sau khi tất cả các lần thử đều thất bại", level=xbmc.LOGINFO)
            with open(cache_file, 'r', encoding='utf-8') as f:
                content = f.read()
            ...
            logging.log(f"Lỗi khi đọc cache cũ: {e}", level=xbmc.LOGERROR)

    # Hiển thị thông báo lỗi thân thiện
    if not check:
        dialog.ok(CONFIG.ADDONTITLE, f'Không thể kết nối đến máy chủ. Vui lòng kiểm tra kết nối mạng của bạn và thử lại sau.')
