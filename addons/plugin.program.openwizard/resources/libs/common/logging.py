...
def log(msg, level=xbmc.LOGDEBUG):
    ...
    # Đảm bảo tương thích với cả Kodi 19, 20 và 21
    try:
        xbmc.log('{0}: {1}'.format(CONFIG.ADDONTITLE, msg), level)
    except:
        # Fallback nếu có lỗi
        xbmc.log('{0}: {1}'.format(CONFIG.ADDONTITLE, msg))
...
def log_notify(title, message, times=2000, icon=CONFIG.ADDON_ICON, sound=False):
    ...
    try:
        ...
        # Fallback nếu có lỗi với notification trong Kodi 21
        xbmc.executebuiltin('Notification({0}, {1}, {2}, {3})'.format(title, message, times, icon))
