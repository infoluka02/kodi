...
def check_build(name, ret):
    ...
    # Sử dụng cơ chế retry và cache mới
    response = tools.open_url(CONFIG.BUILDFILE, max_retries=3, cache=True)

    if not response:
        logging.log("[Check Build] Không thể kết nối đến file build", level=xbmc.LOGINFO)
        return False
...
def check_theme(name, theme, ret):
    ...
    response = tools.open_url(themeurl, max_retries=3, cache=True)

    if not response:
        logging.log("[Check Theme] Không thể kết nối đến file theme", level=xbmc.LOGINFO)
        return False
...
def check_wizard(ret):
    ...
    if not response:
        logging.log("[Check Wizard] Không thể kết nối đến file build", level=xbmc.LOGINFO)
        return False
...
def check_build_update():
    ...
    if not response:
        logging.log("[Check Updates] Không thể kết nối đến file build", level=xbmc.LOGINFO)
        return
    ...
    if len(match) > 0:
        ...
        if version > CONFIG.BUILDVERSION:
            if CONFIG.DISABLEUPDATE == 'false':
                logging.log("[Check Updates] [Phiên bản đã cài đặt: {0}] [Phiên bản hiện tại: {1}] Mở cửa sổ cập nhật".format(CONFIG.BUILDVERSION, version))
                window.show_update_window(CONFIG.BUILDNAME, CONFIG.BUILDVERSION, version, icon, fanart)
            else:
                logging.log("[Check Updates] [Phiên bản đã cài đặt: {0}] [Phiên bản hiện tại: {1}] Cửa sổ cập nhật bị tắt".format(CONFIG.BUILDVERSION, version))
        else:
            logging.log("[Check Updates] [Phiên bản đã cài đặt: {0}] [Phiên bản hiện tại: {1}]".format(CONFIG.BUILDVERSION, version))
    else:
        logging.log("[Check Updates] LỖI: Không thể tìm thấy phiên bản build trong file text", level=xbmc.LOGERROR)
...
def build_count():
    ...
    if not response:
        logging.log("[Build Count] Không thể kết nối đến file build", level=xbmc.LOGINFO)
