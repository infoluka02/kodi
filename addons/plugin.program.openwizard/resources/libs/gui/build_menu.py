...
class BuildMenu:
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('Phiên Bản Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
            directory.add_dir('Menu Lưu Dữ Liệu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_separator()
            directory.add_file('URL cho file txt không hợp lệ', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('Phiên Bản Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('Menu Lưu Dữ Liệu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
        directory.add_separator()
    ...
    def view_build(self, name):
        ...
        if response:
            ...
            directory.add_file('URL cho file txt không hợp lệ', themeit=CONFIG.THEME3)
            directory.add_file('{0}'.format(CONFIG.BUILDFILE), themeit=CONFIG.THEME3)
            return

        if not check.check_build(name, 'version'):
            directory.add_file('Lỗi đọc file txt.', themeit=CONFIG.THEME3)
            directory.add_file('{0} không tìm thấy trong danh sách builds.'.format(name), themeit=CONFIG.THEME3)
            return
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            if updatecheck:
                build = '{0} [COLOR red][PHIÊN BẢN HIỆN TẠI v{1}][/COLOR]'.format(build, CONFIG.BUILDVERSION)
            ...
            directory.add_separator('CÀI ĐẶT')
            directory.add_file('Cài Đặt', {'mode': 'install', 'action': 'build', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME1)
                               
            if guicheck:
                directory.add_file('RESET GUI', {'mode': 'install', 'action': 'gui', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
                                   
            if themecheck:
                directory.add_separator('THEME', fanart=fanart, icon=icon)
                ...
                for themename, themeurl, themeicon, themefanart, themeadult, description in match:
                    ...
                    themetitle = themename if not themename == CONFIG.BUILDTHEME else "[B]{0} (Đã Cài Đặt)[/B]".format(themename)
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('Cài Đặt Mới'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Cài Đặt Thông Thường'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=normal)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Đặt Lại GUI'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=gui)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Thông Tin Build'), 'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME2.format('Cài Đặt {0}'.format(CONFIG.ADDONTITLE)), 'RunPlugin(plugin://{0}/?mode=settings)'.format(CONFIG.ADDON_ID)))
