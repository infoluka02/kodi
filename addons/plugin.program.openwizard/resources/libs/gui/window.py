...
def show_save_data_settings():
    # Thiết lập các giá trị mặc định thay vì hiển thị cửa sổ lựa chọn
    logging.log("[First Run] Setting default save data settings", level=xbmc.LOGINFO)

    # Thiết lập các giá trị mặc định theo yêu cầu
    CONFIG.set_setting('keeptrakt', 'false')      # Không lưu trữ track
    CONFIG.set_setting('keepdebrid', 'false')     # Không lưu trữ Debrid
    CONFIG.set_setting('keeplogin', 'false')      # Không lưu trữ login
    CONFIG.set_setting('keepsources', 'false')    # Không giữ sources
    CONFIG.set_setting('keepprofiles', 'false')   # Không giữ profiles
    CONFIG.set_setting('keepplayercore', 'false') # Không giữ playercore
    CONFIG.set_setting('keepguisettings', 'false') # Không giữ guisettings.xml
    CONFIG.set_setting('keepadvanced', 'false')   # Không giữ advancesettings.xml
    CONFIG.set_setting('keepfavourites', 'false') # Không giữ favourites.xml
    CONFIG.set_setting('keeprepos', 'true')       # Giữ lại repo
    CONFIG.set_setting('keepsuper', 'false')      # Không giữ super favourites
    CONFIG.set_setting('keepwhitelist', 'false')  # Không giữ whitelist
    CONFIG.set_setting('clearcache', 'true')      # Xóa cache
    CONFIG.set_setting('clearpackages', 'true')   # Xóa packages
    CONFIG.set_setting('clearthumbs', 'true')     # Xóa thumbnails
...
def show_build_install_prompt():
    message = "Để cài đặt addon và skin bạn chạy [COLOR lime]Open[/COLOR]Wizard rồi vào mục \"Gói cài đặt\""
    xbmcgui.Dialog().ok(CONFIG.ADDONTITLE, message)

    # Đánh dấu đã hiển thị thông báo
    logging.log("[Build Install Prompt] Showed simple notification to user", level=xbmc.LOGINFO)
    return False
