...
def net_tools():
    directory.add_dir('Kiểm Tra Tốc Độ', {'mode': 'speedtest'}, icon=CONFIG.ICONSPEED, themeit=CONFIG.THEME1)
    if CONFIG.HIDESPACERS == 'No':
        directory.add_separator()
    directory.add_dir('Xem Địa Chỉ IP & MAC', {'mode': 'viewIP'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME1)
...
def save_menu():
    ...
    directory.add_dir('Lưu Dữ Liệu Trakt', {'mode': 'trakt'}, icon=CONFIG.ICONTRAKT, themeit=CONFIG.THEME1)
    directory.add_dir('Lưu Debrid', {'mode': 'realdebrid'}, icon=CONFIG.ICONDEBRID, themeit=CONFIG.THEME1)
    directory.add_dir('Lưu Thông Tin Đăng Nhập', {'mode': 'login'}, icon=CONFIG.ICONLOGIN, themeit=CONFIG.THEME1)
    directory.add_file('Nhập Dữ Liệu Đã Lưu', {'mode': 'managedata', 'name': 'import'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
    directory.add_file('Xuất Dữ Liệu Đã Lưu', {'mode': 'managedata', 'name': 'export'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
    directory.add_file('- Nhấn để chuyển đổi cài đặt -', themeit=CONFIG.THEME3)
...
def enable_addons(all=False):
    ...
    if not all:
        if len(addonids) == 0:
            directory.add_file("Không Tìm Thấy Addon Để Bật Hoặc Tắt.", icon=CONFIG.ICONMAINT)
        else:
            directory.add_file("[I][B][COLOR red]!!Lưu ý: Tắt Một Số Addon Có Thể Gây Ra Vấn Đề!![/COLOR][/B][/I]", icon=CONFIG.ICONMAINT)
            directory.add_dir('Bật Tất Cả Addon', {'mode': 'enableall'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
            for i in range(0, len(addonids)):
                ...
                if tools.get_addon_info(addonids[i], 'name'):
                    state = "[COLOR springgreen][Đã Bật][/COLOR]"
                    goto = "false"
                else:
                    state = "[COLOR red][Đã Tắt][/COLOR]"
                    goto = "true"
...
def remove_addon_data_menu():
    if os.path.exists(CONFIG.ADDON_DATA):
        directory.add_file('[COLOR red][B][XÓA][/B][/COLOR] Tất Cả Dữ Liệu Addon', {'mode': 'removedata', 'name': 'all'}, themeit=CONFIG.THEME2)
        directory.add_file('[COLOR red][B][XÓA][/B][/COLOR] Tất Cả Dữ Liệu Addon Đã Gỡ Cài Đặt', {'mode': 'removedata', 'name': 'uninstalled'}, themeit=CONFIG.THEME2)
        directory.add_file('[COLOR red][B][XÓA][/B][/COLOR] Tất Cả Thư Mục Trống Trong Dữ Liệu Addon', {'mode': 'removedata', 'name': 'empty'}, themeit=CONFIG.THEME2)
        directory.add_file('[COLOR red][B][XÓA][/B][/COLOR] Dữ Liệu Addon {0}'.format(CONFIG.ADDONTITLE), {'mode': 'resetaddon'}, themeit=CONFIG.THEME2)
        directory.add_separator(themeit=CONFIG.THEME3)
...
def developer():
    directory.add_file('Tạo Mã QR', {'mode': 'createqr'}, themeit=CONFIG.THEME1)
    directory.add_file('Kiểm Tra Thông Báo', {'mode': 'testnotify'}, themeit=CONFIG.THEME1)
    directory.add_file('Kiểm Tra Cập Nhật', {'mode': 'testupdate'}, themeit=CONFIG.THEME1)
    directory.add_file('Kiểm Tra Thông Báo Build', {'mode': 'testbuildprompt'}, themeit=CONFIG.THEME1)
    directory.add_file('Kiểm Tra Cài Đặt Lưu Dữ Liệu', {'mode': 'testsavedata'}, themeit=CONFIG.THEME1)
    directory.add_file('Kiểm Tra Phát Hiện Binary', {'mode': 'binarycheck'}, themeit=CONFIG.THEME1)
