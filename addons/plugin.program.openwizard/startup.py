...
def build_update_check():
    ...
    response = tools.open_url(CONFIG.BUILDFILE, check=True, max_retries=3, cache=True)

    if not response:
        logging.log("[Build Check] Không thể kết nối đến file build: {0}".format(CONFIG.BUILDFILE), level=xbmc.LOGINFO)
    elif not CONFIG.BUILDNAME == '':
        if CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy'] and not CONFIG.DEFAULTIGNORE == 'true':
            check.check_skin()

        logging.log("[Build Check] Build đã cài đặt: Đang kiểm tra cập nhật", level=xbmc.LOGINFO)
...
if CONFIG.get_setting('first_install') == 'true':
    ...
    window.show_build_install_prompt()
    window.show_save_data_settings()
else:
    logging.log("[First Run] Skipping Save Data Settings", level=xbmc.LOGINFO)
    if tools.open_url(CONFIG.BUILDFILE, check=True, max_retries=3, cache=True) and CONFIG.get_setting('installed') == 'false':
        logging.log("[Current Build Check] Build chưa được cài đặt", level=xbmc.LOGINFO)
        window.show_build_prompt()
    else:
        logging.log("[Current Build Check] Build đã cài đặt: {0}".format(CONFIG.BUILDNAME), level=xbmc.LOGINFO)
